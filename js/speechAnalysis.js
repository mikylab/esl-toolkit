...
recordBtn.onclick = () => {
  ...
  recognition.onstart = () => { recordBtn.textContent = 'Recording...'; recordBtn.style.backgroundColor = '#ef4444'; recordBtn.disabled = true; };
  ...
  recognition.onend = () => { recordBtn.textContent = 'Start Speaking'; recordBtn.style.backgroundColor = '#3b82f6'; recordBtn.disabled = false; };
  recognition.start();
};
...
function analyzeText(text, tokens) {
  ...
  const advancedWords = Object.keys(wordList).filter(w => ['C1','C2'].includes(wordList[w]) && !tokens.includes(w));
  ...
  cefrChart = new Chart(ctx, {
    ...
    options: { responsive:true, plugins:{legend:{display:false}, tooltip:{callbacks:{label:function(context){const lvl=context.label; const words=wordsByLevel[lvl]||[]; return `${words.length} word(s): ${words.join(', ')}`;}}}}, scales:{y:{beginAtZero:true, precision:0}}}
  });
}

// ---- Repeated words chart ----
let repeatsChart; // global
...
function updateRepeatedWordsChart(tokens) {
  ...
  repeatsDiv.innerHTML = ''; // clear previous bubbles
...
repeatedEntries.forEach(([word, count]) => {
  ...
  // Bubble size scales with repetition count
...
async function checkGrammar(sentence) {
    ...
    try {
        ...
        if (data.matches.length > 0) {
            ...
            // Optional: call a function to store or highlight correct sentence
